...
extern double Fraction=0.0; // fraction of level height for midpoint to qualify
extern double Factor=2.0; // factor of level height to qualify as breakout 
//---- buffers
...
int start()
{
 int i,j,counted_bars,limit,shift1,shift2,shift3;datetime bartime;
 ...
 double vupper,vlower,vbuffer,midpoint;
...
// else counted_bars=1;
 ...
 { 
  ...
  if(i>Bars-1-BarMin) continue;   

  shift1=iBarShift(NULL,TimeFrame,Time[i],false);

  if(shift1<0) continue;
  
  open=iOpen(NULL,TimeFrame,shift1);
  close=iClose(NULL,TimeFrame,shift1);
  high=iHigh(NULL,TimeFrame,shift1);
...
// Create new levels
  ...
  shift2=iHighest(NULL,TimeFrame,MODE_CLOSE,BarMin,shift3);
  closeH=iClose(NULL,TimeFrame,shift2);
  shift2=iLowest(NULL,TimeFrame,MODE_CLOSE,BarMin,shift3);
  closeL=iClose(NULL,TimeFrame,shift2); 

  shift2=iHighest(NULL,TimeFrame,MODE_OPEN,BarMin,shift3);
  openH=iOpen(NULL,TimeFrame,shift2);
  shift2=iLowest(NULL,TimeFrame,MODE_OPEN,BarMin,shift3);
  openL=iOpen(NULL,TimeFrame,shift2);
  
  vhigh=MathMax(closeH,openH);
  vlow=MathMin(closeL,openL);
  vdiff=vhigh-vlow;

  if(vdiff<bufferlevelPoints) continue;
  ...
  if(peakDemand || peakSupply)
  {
   ...
   time=TimeToStr(iTime(NULL,TimeFrame,shift1),TIME_DATE|TIME_MINUTES);
...
//    if(close<price1) price1=close-bufferPoints; 
...
//    if(close>price2) price2=close+bufferPoints; 
