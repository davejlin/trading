...
int init()
{
   ...
   SetIndexBuffer(6,IndexVolume); SetIndexLabel(6,"Volume");SetIndexStyle(6,DRAW_NONE);   
...
int start()
{
   ...
   for (i=limit; i>=0; i--)
   {
      ...
      for (int k=0; k<numPairs; k++)
      {
         ...
         idxVolume += indexVolume(k,y);
      }
      IndexOpen[i]  = idxOpen;
      IndexHigh[i]  = idxHigh;
      IndexLow[i]   = idxLow;
      IndexClose[i] = idxClose;
      IndexVolume[i]= idxVolume;
   }
...
double indexVolume(int j, int i)
{
 return(iVolume(symbols[j],0,i));
}