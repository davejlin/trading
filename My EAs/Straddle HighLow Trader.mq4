...
extern int Straddle=38;              // pips for straddle based on previous bar's high/low
extern int ReEntry=17;               // pips from previous bar's high/low at which to re-enter complete straddle (use negative number if no re-entry is desired)
...
extern int SLProfit2=-1;             // pips profit after which to move SL (#2) (enter negative value to disable)
extern int SLMove2=10;               // pips to move SL to BE+SLMove after SLProfit is reached (#2)
extern int SLProfit3=-1;             // pips profit after which to move SL (#3) (enter negative value to disable)
...
string strL="StraddleHighLow Long",strS="StraddleHighLow Short";
int code=1;
datetime ot,lasttime;
double high,low,ReEntryH,ReEntryL;
//+------------------------------------------------------------------+
...
//----
 ...
 comment  =StringConcatenate(pd," StraddleHighLow"); 
...
void SubmitOrders()
{
 ...
 { 
  high=iHigh(NULL,0,1);
  low=iLow(NULL,0,1);   
  double priceH=NormDigits(high+NormPoints(Straddle-1));
  double priceL=NormDigits(low-NormPoints(Straddle+1));
  if(Bid<=priceH&&Bid>=priceL) // new bar straddle, make sure there's legitimate price entry window if EA started off-bar 
  {
   if(LongPendOrder||(!LongPendOrder&&high!=iHigh(NULL,0,2))) // avoid identical re-entries if pre-existing
   {
    EntryPrice=NormDigits(high+NormPoints(Straddle)+(Ask-Bid));
    SL=StopLong(EntryPrice,StopLoss);
    SendPending(Symbol(),OP_BUYSTOP,Lots,EntryPrice,Slippage,SL,0,comment,magic,0,Blue);
    ot=TimeCurrent();     
   }
   if(ShortPendOrder||(!ShortPendOrder&&low!=iLow(NULL,0,2))) // avoid identical re-entries if pre-existing
   {
    EntryPrice=NormDigits(low-NormPoints(Straddle));   
  ...
  {
   ReEntryH=NormDigits(high-NormPoints(ReEntry));
   ReEntryL=NormDigits(low+NormPoints(ReEntry));
  }   
 }
 else if(ReEntry>=0)
 {
