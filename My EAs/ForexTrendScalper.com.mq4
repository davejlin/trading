...
extern string Current_Version="1.01";
...
extern bool   Use_Hour_Trade=false;
  ...
  else 
  {
   ...
   { 
    ...
    { 
     price=NormDigits(Bid-(i*Order_Spacing_p));
     Lots=CalcLots(NormDigits(price-SG),true);   
     SendPending(Symbol(),OP_BUYLIMIT,price,Lots,slippage,0,0,comment,magicN); 
     AddSLTP(SG,LG);
     ...
     price=NormDigits(Bid+(i*Order_Spacing_p));
     Lots=CalcLots(NormDigits(LG-price),true);   
     SendPending(Symbol(),OP_SELLLIMIT,price,Lots,slippage,0,0,comment,magicN);
     spread=NormDigits(Ask-Bid);
     lg=NormDigits(LG+spread);
     sg=NormDigits(SG+spread);
     AddSLTP(lg,sg);
...
void Initialization()
{
 ...
 {      
  ...
  if (!AT)
  {
   Alert("This EA is not licensed for live trading. Please contact customer support.");
  }
 }
 else 
 {
...
void PrintComments()
{
 ...
 sComment = sComment + "Total Lots: " + DoubleToStr(totallots,2) + nl;
 sComment = sComment + "Total Daily Profit: " + DoubleToStr(dailyprofit,2) + nl; 
