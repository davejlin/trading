...
{
 double sl,targetorderprofit,orderprofit[];
 int trade,trades=OrdersTotal(),totalNSymbol; 
 ...
 ArrayResize(orderprofit,totalNSymbol); // only size it to the current chart's orders 
 ...
 for(trade=trades-1;trade>=0;trade--)
 {
  OrderSelect(trade,SELECT_BY_POS,MODE_TRADES);
  if(OrderSymbol()!=Symbol()) continue;
  orderprofit[n]=OrderProfit();  
  n++;
 } 

 if(LetItRide<0)
 {
  ArraySort(orderprofit);
  targetorderprofit=orderprofit[0];
 }
 else if(LetItRide>0)
 {
  ArraySort(orderprofit,WHOLE_ARRAY,0,MODE_DESCEND);
  targetorderprofit=orderprofit[0];
 }
 ...
 if(long)
 {
  for(trade=trades-1;trade>=0;trade--)
  {
   OrderSelect(trade,SELECT_BY_POS,MODE_TRADES);
   if(OrderSymbol()!=Symbol()) continue;  
   if(OrderProfit()!=targetorderprofit) ExitOrder(true,false); 
   else 
   {
    if(StopLossRide>0)
    {
 ...
 else
 {
  for(trade=trades-1;trade>=0;trade--)
  {
   OrderSelect(trade,SELECT_BY_POS,MODE_TRADES);
   if(OrderSymbol()!=Symbol()) continue;  
   if(OrderProfit()!=targetorderprofit) ExitOrder(false,true); 
   {
    if(StopLossRide>0)
    {
...
double PP;
 ...
 if(IsTesting()) return;
 int i,j,k,lastj,offseti,limit,lotmod, counted_bars = IndicatorCounted();
 ...
 if(!init1)
 {
  ...
  if(!init2) // dynamic update of lots 7/18/09 
  {
   ...
   if(NElements!=0)
   {
    lotsi=AccountEquity()*0.01/value;
    if(StringFind(Symbol(),"XAGUSD")>0) // for Silver
    {
     lotsi=0.02*lotsi;
     lotmod=lotsi/lotsmin;
     lotsi=MathMax(lotsmin,lotmod*lotsmin); 
    }
   }    
   else lotsi=0;
