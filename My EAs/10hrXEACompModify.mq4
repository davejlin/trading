...
//|                                                                      |
//|Modify orders if next trigger in the same direction                   |                                                  |
//|                                                                      |
...
//      
  if(AskPrice>=TriggerPrice10hrX&&flag_10hrXLongCross==true&&Window10hrX>=0&&checktime<=0)
  {
   ...
   if(OrdersTotal()!=0) // check if exisiting longs
   {
    OrderSelect(0,SELECT_BY_POS,MODE_TRADES);
    OrderModify(OrderTicket(),OrderOpenPrice(),SL,TakeLong(AskPrice,TakeProfit_10hrXMain),0,Blue);
   }
   else
...
//
  if(BidPrice<=TriggerPrice10hrX&&flag_10hrXShortCross==true&&Window10hrX>=0&&checktime<=0)
  {
   ...
   if(OrdersTotal()!=0) // check if exisiting shorts
   {
    OrderSelect(0,SELECT_BY_POS,MODE_TRADES);
    OrderModify(OrderTicket(),OrderOpenPrice(),SL,TakeShort(AskPrice,TakeProfit_10hrXMain),0,Red);
   }
   else  
