...
extern int LookBackPeriod=10;        // number of bars to look-back to determine swing high/low
extern int WindowofOpportunity=5;    // number of bars after cross for price to exceed swing high/low

extern int MAperiodS=5;              // short-term MA period (trigger)      
extern int MAperiodM=13;             // medium-term MA period (trigger)
extern int MAperiodL=34;             // long-term MA period (filter)

extern double PART =0.020;           // percentage of account balance at risk per trade
extern double PARTA=0.080;           // percentage of account balance at risk per total account (aggregate open order risk in entire account)
                     
extern int TakeProfit=-1;            // pips initial TP (use negative number if not desired)
extern int StopLoss=15;              // pips initial SL (use negative number if not desired)
...
extern int SLMove=8;                 // pips to move SL to BE+SLMove after SLProfit is reached
...
//----
 ...
 comment  =StringConcatenate(pd," Pip Sniper"); 
...
void SubmitOrders()
{
 ...
 if(prevHigh>target)
 {
  for(i=1;i<=WindowofOpportunity;i++)
  {
   ...
   mal1=iMA(NULL,0,MAperiodL,MAshiftL,MAmethodL,MApriceL,i); 
    
   if(mas1>=mal1 && mam1>=mal1)
   {  
