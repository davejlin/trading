...
//|                                                                      |
//|Added 4th trigger June 25, 2008                                       |
//|Added break-even function July 2, 2008                                |
//+----------------------------------------------------------------------+
...
extern bool Entry1=false;           // true = activate entry method 1
extern bool Entry2=false;           // true = activate entry method 2
extern bool Entry3=false;           // true = activate entry method 3
extern bool Entry4=true;            // true = activate entry method 4
...
extern double Lots41=0.01;         // Trigger 4 Order 1 lottage per trade
extern double Lots42=0.01;         // Trigger 4 Order 2 lottage per trade
extern double Lots43=0.01;         // Trigger 4 Order 3 lottage per trade
...
extern int TakeProfit41=5;         // Trigger 4 Order 1 pips desired TP
extern int TakeProfit42=5;        // Trigger 4 Order 2 pips desired TP
extern int TakeProfit43=5;         // Trigger 4 Order 3 pips desired TP
...
extern int StopLoss41=-1;           // Trigger 4 Order 1 pips desired SL
extern int StopLoss42=-1;           // Trigger 4 Order 2 pips desired SL
extern int StopLoss43=-1;           // Trigger 4 Order 3 pips desired SL 
...
extern int TrailProfit41=0;         // Trigger 4 Order 1 pips desired TrailProfit
extern int TrailMove41=0;           // Trigger 4 Order 1 pips desired TrailMove
extern int TrailProfit42=0;         // Trigger 4 Order 2 pips desired TrailProfit
extern int TrailMove42=0;           // Trigger 4 Order 2 pips desired TrailMove
extern int TrailProfit43=0;         // Trigger 4 Order 3 pips desired TrailProfit
extern int TrailMove43=0;           // Trigger 4 Order 3 pips desired TrailMove

                                    // BreakEven:  pips profit at which to set stops to break-even 
                                    // (it should be a number greater than 1 to make sense)
                                    // (use zero or negative number to turn off this function)

extern int BreakEven11=10;          // Trigger 1 Order 1 pips at which to set stops to break-even
extern int BreakEven12=10;          // Trigger 1 Order 2 pips at which to set stops to break-even
extern int BreakEven13=10;          // Trigger 1 Order 3 pips at which to set stops to break-even
extern int BreakEven21=10;          // Trigger 2 Order 1 pips at which to set stops to break-even
extern int BreakEven22=10;          // Trigger 2 Order 2 pips at which to set stops to break-even
extern int BreakEven23=10;          // Trigger 2 Order 3 pips at which to set stops to break-even
extern int BreakEven31=10;          // Trigger 3 Order 1 pips at which to set stops to break-even
extern int BreakEven32=10;          // Trigger 3 Order 2 pips at which to set stops to break-even
extern int BreakEven33=10;          // Trigger 3 Order 3 pips at which to set stops to break-even
extern int BreakEven41=10;          // Trigger 4 Order 1 pips at which to set stops to break-even
extern int BreakEven42=10;          // Trigger 4 Order 2 pips at which to set stops to break-even
extern int BreakEven43=10;          // Trigger 4 Order 3 pips at which to set stops to break-even
...
extern datetime StartDate=D'2008.05.01';
// Internal usage parameters:
int Slippage=3,bo=1;
double Lots[4,3];
int magic[4,3],TakeProfit[4,3],StopLoss[4,3],TrailProfit[4,3],TrailMove[4,3],Norder[4,3],BreakEven[4,3];
string comment[4,3];
datetime ot[4];
int lotsprecision=2;
...
int code=1,Nmethods=4,Norders=3;
bool LongExit,ShortExit;//LongExitSL,ShortExitSL;
datetime lasttime,tlastViolateS,tlastViolateR,tlastFormS,tlastFormR;
...
bool LastFractRS; // flag to keep track of the bias of the last Fractal RS line to form (for Trigger 4)
...
//----
 ...
 StopLoss[3,0]=StopLoss41;
 StopLoss[3,1]=StopLoss42;
 StopLoss[3,2]=StopLoss43;   
 ...
 TakeProfit[3,0]=TakeProfit41;
 TakeProfit[3,1]=TakeProfit42;
 TakeProfit[3,2]=TakeProfit43;    
 ...
 TrailProfit[3,0]=TrailProfit41;
 TrailProfit[3,1]=TrailProfit42;
 TrailProfit[3,2]=TrailProfit43; 
 ...
 TrailMove[3,0]=TrailMove41;
 TrailMove[3,1]=TrailMove42;
 TrailMove[3,2]=TrailMove43; 

 BreakEven[0,0]=BreakEven11;
 BreakEven[0,1]=BreakEven12;
 BreakEven[0,2]=BreakEven13;
 BreakEven[1,0]=BreakEven21;
 BreakEven[1,1]=BreakEven22;
 BreakEven[1,2]=BreakEven23;
 BreakEven[2,0]=BreakEven31;
 BreakEven[2,1]=BreakEven32;
 BreakEven[2,2]=BreakEven33; 
 BreakEven[3,0]=BreakEven41;
 BreakEven[3,1]=BreakEven42;
 BreakEven[3,2]=BreakEven43; 
 ...
 magic[3,0] =230000+Period(); 
 magic[3,1] =230001+Period();
 magic[3,2] =230002+Period();  
 ...
 comment[3,0]  =StringConcatenate(pd," FractRS 41"); 
 comment[3,1]  =StringConcatenate(pd," FractRS 42"); 
 comment[3,2]  =StringConcatenate(pd," FractRS 43"); 
...
void Trigger4()
{ 
 int i,checktime=iBarShift(NULL,0,ot[3],false);
 if(checktime<bo) return;

// if(Norder[3,0]!=0 || Norder[3,1]!=0 || Norder[3,2]!=0) return; // one order set at a time 

 double lots,SL,TP; 

 if(LastFractRS && Bid==iLow(NULL,0,FractDowni))
 {
  if(iTime(NULL,0,FractDowni)>tlastFormR)// && iTime(NULL,0,FractDowni)>ot[3]) // fractals must form after Fractal RS line & last order
  { 
   for(i=0;i<Norders;i++)
   {
    lots=Lots[3,i];
    if(lots<=0) continue;
    if(StopLoss[3,i]<0) SL=StopLong(Ask,StopLossSafety);
    else SL=StopLong(Ask,StopLoss[3,i]);
    TP=TakeLong(Ask,TakeProfit[3,i]);
    SendOrderLong(Symbol(),lots,Slippage,SL,TP,comment[3,i],magic[3,i],0,Blue);
   }
   ot[3]=TimeCurrent();
  }
 }
 
 if(!LastFractRS && Bid==iHigh(NULL,0,FractUpi))
 {
  if(iTime(NULL,0,FractUpi)>tlastFormS)// && iTime(NULL,0,FractUpi)>ot[3]) // fractals must form after Fractal RS line & last order
  {  
   for(i=0;i<Norders;i++)
   {   
    lots=Lots[3,i];
    if(lots<=0) continue;     
    if(StopLoss[3,i]<0) SL=StopShort(Bid,StopLossSafety);
    else SL=StopShort(Bid,StopLoss[3,i]);
    TP=TakeShort(Bid,TakeProfit[3,i]);
    SendOrderShort(Symbol(),lots,Slippage,SL,TP,comment[3,i],magic[3,i],0,Red); 
   }
   ot[3]=TimeCurrent();
  }
 }
...
//    LongExitSL=true;
    Trig3ShortTarget=EMPTY_VALUE;
    LastFractRS=false; // flag to keep track of the bias of the last Fractal RS line to form (for Trigger 4)
    tlastFormS=TimeCurrent(); // keep track of when the last Fractal RS line was formed (for Trigger 4)
   }
  }
...
//    ShortExitSL=true;    
    Trig3LongTarget=EMPTY_VALUE;  
    LastFractRS=true; // flag to keep track of the bias of the last Fractal RS line to form (for Trigger 4)   
    tlastFormR=TimeCurrent(); // keep track of when the last Fractal RS line was formed (for Trigger 4)
   } 
  }
...
{
 int i;double close,open,high,low;
 switch(flag)
 {
  case 0:
  ...
  case 1:
   if(OrderType()==OP_SELL)
   {
    for(i=shift+2;i<1000;i++)
    {
     ...
     high=iHigh(NULL,0,i);
     if(open>high && close>high) ExitOrder(false,true,2);
   ...
   else if(OrderType()==OP_BUY)
   {
    for(i=shift+2;i<1000;i++)
    {
     ...
     close=iClose(NULL,0,1);
     low=iLow(NULL,0,i);
     if(open<low && close<low) ExitOrder(true,false,2);
...
void ManageOrders()
{
 ...
 int mn,trade,trades=OrdersTotal(); 
...
//+------------------------------------------------------------------+
void FixedStopsBE(int PP,int PFS)
{
  if(PFS<0) return;
...
{   
 ...
 else if(mn==magic[3,0]) ot[3]=OrderOpenTime();
 else if(mn==magic[3,1]) ot[3]=OrderOpenTime();
 else if(mn==magic[3,2]) ot[3]=OrderOpenTime(); 
