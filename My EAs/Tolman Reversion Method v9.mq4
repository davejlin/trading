...
//| April 22, 2012 - v6: add two Weekly high/low lines, high/low/Envelope|
//|                  filter, daily ATR and ATR risk display,             |
//|                  MaxNumberTradesPair                                 |
//| July 18, 2012 - v7: ATRLotsRisk is now always used for lot size      |
//| July 21, 2012 - v8: ATR-based initial SL & toggle                    |
//| July 23, 2012 - v9: if no elements in history, then lotsn=1          |
//+----------------------------------------------------------------------+
#property copyright "Copyright © 2009-2012, Todd Tolman & David J. Lin"
...
extern double Risk=1;           // total percentage drawdown allowed for all open orders based on average entry price 
...
extern bool StopLossATR=true;   // true: use D1 ATR for initial SL, false: use StopLoss paramter for initial SL
extern int MATrail=5;           // pips from MA line to exit 
                                // set to 0 to exit at MA line
                                // set to a negative number to not exit at MA line
extern int LetItRide=-1;         // set to 0 to exit all positions at MA line
...
extern int StopLossRide=40;      // pips from price (Ask for longs, Bid for shorts) for stoploss for riding order
...
extern int WeekHLPeriodFilter=4; // bars in window of consideration for weekly high/low and Envelope entry filter (use 0 or negative number to turn off filter)
extern int WeekHLPeriod1=4;      // bars in first window of consideration of weekly high/low (use 0 or negative number to turn off line graphing)
extern int WeekHLPeriod2=11;     // bars in second window of consideration of weekly high/low (use 0 or negative number to turn off line graphing)
extern color WeekHLColor1=Blue;  // color for first window weekly high/low chart line 
extern color WeekHLColor2=Red;   // color for second window weekly high/low chart line
extern int WeekHLLineWidth1=1;           // width of weekly high/lowlines
extern int WeekHLLineWidth2=1;           // width of weekly high/lowlines
extern int WeekHLLineStyle1=STYLE_SOLID; // style of weekly high/low lines (STYLE_SOLID, STYLE_DASH, STYLE_DOT, STYLE_DASHDOT, STYLE_DASHDOTDOT)
extern int WeekHLLineStyle2=STYLE_SOLID; // style of weekly high/low lines (STYLE_SOLID, STYLE_DASH, STYLE_DOT, STYLE_DASHDOT, STYLE_DASHDOTDOT)

extern int ATRPeriodD1=14;       // bars period of D1 ATR
extern int ATRBasisD1=0;         // 0 for current unclosed D1 bar, 1 for last closed D1 bar, 2 for 2nd to last closed D1 bar, etc ...
...
extern int MaxNumberTradesPair=1; // maximum number of trades allowed per currency pair in account, if exceeded, currency pair's EA is prohibited from submitting new trades 
//---- buffers
int top=100;
int handle;
double mult,lotsi;
...
datetime ots,otl,lasttime,lastD1time;
double lotsmin,lotsmax,MATrailPoints,StopLossPoints,TakeProfitPoints,StopLossRidePoints;
int lotsprecision;
int Slippage=5;
int NTradesGlobal,NTradesPair;

double totalLotsXPrice,totalLots,aveEntry,aveSL;
bool direction;

double ATRD1,ATRLotsRisk;
double weekHigh1,weekHigh2,weekHighF,weekLow1,weekLow2,weekLowF;
double uEnv,lEnv;
int WeekHLStart;
string NameWHigh1="Weekly High1",NameWHigh2="Weekly High2",NameWLow1="Weekly Low1",NameWLow2="Weekly Low2";
//+------------------------------------------------------------------+
...
//---- 
 ...
 if(Corner==1)
 {
  ...
  MakeLabel( "ATRD1t", 75, top+195 );    
  MakeLabel( "ATRD1v", 10, top+195 );  
  MakeLabel( "ATRLotsRiskt", 75, top+215 );    
  MakeLabel( "ATRLotsRiskv", 10, top+215 );              
 }
 else if(Corner==2)
 {
 ...
 ObjectSetText( "ATRD1t",  "ATR D1:", 10, "Arial", TitleColor );  

 ObjectSetText( "ATRLotsRiskt",  "Lots ATR:", 10, "Arial", TitleColor );  
 ...
 lastD1time=iTime(NULL,PERIOD_D1,0);
...
//---- 
 ...
 ObjectDelete("ATRD1t"); 
 ObjectDelete("ATRD1v");      
 ObjectDelete("ATRLotsRiskt"); 
 ObjectDelete("ATRLotsRiskv"); 

 if(ObjectFind(NameWHigh1)>=0) ObjectDelete(NameWHigh1);
 if(ObjectFind(NameWHigh2)>=0) ObjectDelete(NameWHigh2);
 if(ObjectFind(NameWLow1)>=0) ObjectDelete(NameWLow1);
 if(ObjectFind(NameWLow2)>=0) ObjectDelete(NameWLow2); 
//----
 return(0);
}
//+------------------------------------------------------------------+
...
//----   
 ...
 if(lastD1time==iTime(NULL,PERIOD_D1,0)) return(0); 
 lastD1time=iTime(NULL,PERIOD_D1,0); 
 WeekHLUpdate();
//----
 return(0);
...
{ 
 ...
 uEnv=iEnvelopes(NULL,0,EnvPeriod,EnvMethod,0,0,EnvDev,MODE_UPPER,0);
 lEnv=iEnvelopes(NULL,0,EnvPeriod,EnvMethod,0,0,EnvDev,MODE_LOWER,0);
 double open=iOpen(NULL,0,0);

 if(open>lEnv && Bid<lEnv && Bias>=0 && WeekFilter(true))
 {
  SL=StopLoss(true);
 ...
 if(open<uEnv && Bid>uEnv && Bias<=0 && WeekFilter(false))
 {
  SL=StopLoss(false);
...
//+------------------------------------------------------------------+
double StopLoss(bool flag)
{
 if(flag)
 {
  if(StopLossATR) return(StopLong(Ask,ATRD1));
  else return(StopLong(Ask,StopLossPoints));
 }
 else
 {
  if(StopLossATR) return(StopShort(Bid,ATRD1));
  else return(StopShort(Bid,StopLossPoints));
 }
}
//+------------------------------------------------------------------+
...
{   
 ...
 // D1 ATR
 ATRD1=iATR(NULL,PERIOD_D1,ATRPeriodD1,ATRBasisD1);
 ObjectSetText( "ATRD1v", DoubleToStr(ATRD1/PP,2), 10, "Arial", ValueColor );

 // ATR Lot Risk:
 ATRLotsRisk=CalcLots(ATRD1,Risk);
 ObjectSetText( "ATRLotsRiskv", DoubleToStr(ATRLotsRisk,lotsprecision), 10, "Arial", ValueColor );
...
int CheckNumberOrder() // check number of orders in account regardless of origin
{
 ...
 for(trade=trades-1;trade>=0;trade--)
 {
  ...
  OrderSelect(trade,SELECT_BY_POS,MODE_TRADES);
  if(OrderSymbol()!=Symbol()) continue;
  if(OrderMagicNumber()==Magic) triggered=true;
  NTradesPair++;
...
//+------------------------------------------------------------------+
bool WeekFilter(bool flag)
{
 if(WeekHLPeriodFilter>0)
 {
  if(flag)
  {
   if(weekLowF>lEnv) return(false);
  }
  else
  {
   if(weekHighF<uEnv) return(false);
  }
 }
...
bool TradeFilter()
{
 if(triggered) return(true);                        // specific to this EA
 if(NTradesGlobal>=MaxNumberTrades) return(true);   // global in entire account 
 if(NTradesPair>=MaxNumberTradesPair) return(true); // specific to this currency pair
...
void WeekHLUpdate()
{
 if(WeekHLPeriodFilter>0) weekHighF=NormDigits(iHigh(NULL,PERIOD_W1,iHighest(NULL,PERIOD_W1,MODE_HIGH,WeekHLPeriodFilter,WeekHLStart)));
 if(WeekHLPeriod1>0) weekHigh1=NormDigits(iHigh(NULL,PERIOD_W1,iHighest(NULL,PERIOD_W1,MODE_HIGH,WeekHLPeriod1,WeekHLStart)));
 if(WeekHLPeriod2>0) weekHigh2=NormDigits(iHigh(NULL,PERIOD_W1,iHighest(NULL,PERIOD_W1,MODE_HIGH,WeekHLPeriod2,WeekHLStart)));
 
 if(WeekHLPeriodFilter>0) weekLowF=NormDigits(iLow(NULL,PERIOD_W1,iLowest(NULL,PERIOD_W1,MODE_LOW,WeekHLPeriodFilter,WeekHLStart)));
 if(WeekHLPeriod1>0) weekLow1=NormDigits(iLow(NULL,PERIOD_W1,iLowest(NULL,PERIOD_W1,MODE_LOW,WeekHLPeriod1,WeekHLStart)));
 if(WeekHLPeriod2>0) weekLow2=NormDigits(iLow(NULL,PERIOD_W1,iLowest(NULL,PERIOD_W1,MODE_LOW,WeekHLPeriod2,WeekHLStart)));
...
void UpdateLine()
{
 if(WeekHLPeriod1>0) DrawLine(weekHigh1,iTime(NULL,0,0),iTime(NULL,0,1),NameWHigh1,WeekHLColor1,WeekHLLineWidth1,WeekHLLineStyle1);
 if(WeekHLPeriod2>0) DrawLine(weekHigh2,iTime(NULL,0,0),iTime(NULL,0,1),NameWHigh2,WeekHLColor2,WeekHLLineWidth2,WeekHLLineStyle2);  
 if(WeekHLPeriod1>0) DrawLine(weekLow1,iTime(NULL,0,0),iTime(NULL,0,1),NameWLow1,WeekHLColor1,WeekHLLineWidth1,WeekHLLineStyle1);
 if(WeekHLPeriod2>0) DrawLine(weekLow2,iTime(NULL,0,0),iTime(NULL,0,1),NameWLow2,WeekHLColor2,WeekHLLineWidth2,WeekHLLineStyle2); 
 return;
}
//+------------------------------------------------------------------+
void DrawLine(double price, int time1, int time2, string name, color clr, int width, int style)
{
 ...
 ObjectSetText(name,"Price: "+DoubleToStr(price,Digits));
 return;
}