...
//|  - MACD: one open MACD order at a time, ignore future triggers       |
//|          if previous order in same direction is still open           |
//|                                                                      |
...
extern double LotsMACDUnload=10.0;   // lots to unload MACD (fractional values ok)
...
extern int StopLoss_MACD=70;         // pips stop loss for Secret MACD main order
...
bool flag_MACDbuy=false;             // flag to gauge whether a MACD buy order is open
bool flag_MACDsell=false;            // flag to gauge whether a MACD sell order is open
...
// Keep track of MACD orders to ensure only 1 MACD order is open at a time
  ...
  else if(OrderMagicNumber()==magic2&&OrderType()==OP_SELL)
  {
   flag_MACDsell=true;
   flag_MACDbuy=false;
   MACDtime=OrderOpenTime();
...
//      
  ...
  {
   CloseShorts(magic1);
   OrderSend(Symbol(),OP_BUY,Lots,Ask,Slippage,StopLong(Ask,StopLoss_MA),TakeLong(Ask,TakeProfit_MAMain),NULL,magic1,0,Blue);
   flag_dump1=true;
  }//Long
...
//
  ...
  {
   CloseLongs(magic1); 
   OrderSend(Symbol(),OP_SELL,Lots,Bid,Slippage,StopShort(Bid,StopLoss_MA),TakeShort(Bid,TakeProfit_MAMain),NULL,magic1,0,Red);
   flag_dump1=true;
  }//Shrt
 }
...
//
//Enter MACD Long, Exit Short 
//    
  if(base1>signal1 && base2<signal2 && signal1<-0.0010 && timeleft<0)
  {
   CloseShorts(magic2);        
   if(!flag_MACDbuy) // only submit new long MACD order is there are no open long MACD orders
   {
...
//
//Enter MACD Short, Exit Long
//
  if(base1<signal1 && base2>signal2 && signal1>0.0010 && timeleft<0)
  {
   CloseLongs(magic2); 
   if(!flag_MACDsell)// only submit new short MACD order is there are no open short MACD orders
   {
...
void TakeProfitUnload() // Unload LotsUnload at TakeProfit_MACDUnload and/or LotsUnloadMACD at TakeProfit_MACDUnload
{
 if(flag_dump1) 
 {
  if(TakeProfit_MAUnload==0||LotsUnload==0.0)
   return;  
  flag_dump1=TakeProfitCycle(flag_dump1,LotsUnload,magic1,TakeProfit_MAUnload);
 }
