...
// StraddleOrderLimit.mq4 

// This "initiator" EA applies a straddle buy-sell limits upon application.
// Double straddle applied if Lots2 is set to non-zero values.

// coded by David J. Lin (d-lin@northwestern.edu), Evanston, IL, September 15, 2006 
...
//Immediately straddle the trade:
  ...
  OrderSend(Symbol(),OP_BUYLIMIT,Lots1,entry,Slippage,StopLong(entry,StopLoss1),TakeLong(entry,TakeProfit1),NULL,1,0,Blue);
      entry = Bid+(Sell1*Point);
  OrderSend(Symbol(),OP_SELLLIMIT,Lots1,entry,Slippage,StopShort(entry,StopLoss1),TakeShort(entry,TakeProfit1),NULL,2,0,Red); 
  
  if (Lots2!=0.0)
  {
      entry = Ask-(Buy2*Point);
   OrderSend(Symbol(),OP_BUYLIMIT,Lots2,entry,Slippage,StopLong(entry,StopLoss2),TakeLong(entry,TakeProfit2),NULL,3,0,Green);
      entry = Bid+(Sell2*Point);
   OrderSend(Symbol(),OP_SELLLIMIT,Lots2,entry,Slippage,StopShort(entry,StopLoss2),TakeShort(entry,TakeProfit2),NULL,4,0,Orange); 
  }
