...
// Timeframe settings:
extern string TimeFrame_fast="M1";  // fast period string 
extern string TimeFrame_slow="M5";  // slow period string 
// Order Management settings:
extern double Lots=0.01;             // lot size per order 
extern int StopLossOrder=100;        // pips catastrophic stop loss (tacked on actual order)
extern int StopLoss=10;              // pips stop loss stealth (use 0 to deactivate)
extern int TakeProfit=25;            // pips take profit stealth (use 0 to deactivate)
extern int TrailBegin=9;             // pips profit to begin trail (use 0 to deactivate)
extern int Trail=4;                  // pips trail stealth (begins after TrailBegin pips attained)
...
// Exit Signal Toggles:
// Fast timeframe:
extern bool Exit_Fast_Signals=true;          // true = use exit critera on fast period 
...
// Slow timeframe:
extern bool Exit_Slow_Signals=true;          // true = use exit critera on slow period 
...
// exit parameters
extern int TimeDelay=5;                      // minutes time delay before a new order can be taken after an exit-criteria-based exit (includes stealth stop-loss, take-profit, trail exits, but not catastrophic-stop-loss exits)
// color parameters
...
void ExitCheck()
{
 if(OrderType()==OP_BUY)
 {
  if(ExitCriteria(true))
  {
   ...
   Alert(Symbol()," long exit criteria #",OrderTicket());
  }
 }
 else if(OrderType()==OP_SELL)
 {
  if(ExitCriteria(false))
  {
   ...
   Alert(Symbol()," short exit criteria #",OrderTicket());
  }  
 } 
 return;
}
...
void StealthSLTP()
{
 if(StopLoss>0)
 {
  if(OrderType()==OP_BUY)
  {
   ...
   {
    ExitOrder(true,false); 
    Alert(Symbol()," long stealth SL exit #",OrderTicket());    
   }
  }
  else if(OrderType()==OP_SELL)
  {
   ...
   { 
    ExitOrder(false,true);   
    Alert(Symbol()," short stealth SL exit #",OrderTicket());       
   }
  }
 }
 ...
 if(TakeProfit>0)
 {
  if(OrderType()==OP_BUY)
  {
   ...
   {
    ExitOrder(true,false); 
    Alert(Symbol()," long stealth TP exit #",OrderTicket());       
   }
  }
  else if(OrderType()==OP_SELL)
  {
   ...
   {
    ExitOrder(false,true);   
    Alert(Symbol()," short stealth TP exit #",OrderTicket());       
   }
  } 
 } 
 return;
...
void StealthTrail()
{
 if(TrailBegin>0)
 {
  if(OrderType()==OP_BUY)
  {
   ...
   {
    ExitOrder(true,false);
    Alert(Symbol()," long stealth trail exit #",OrderTicket());        
   }
  ...
  else if(OrderType()==OP_SELL)
  {
   ...
   {
    ExitOrder(false,true);
    Alert(Symbol()," short stealth trail exit #",OrderTicket());       
   }   
